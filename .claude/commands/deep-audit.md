...
| 阶段 | 步骤 | 预计时间 |
|------|------|----------|
| 探索阶段 | Step 1-3 | 5-10 分钟 |
| 分析阶段 | Step 4-6 | 5-10 分钟 |
| 修复阶段 | Step 7-9 | 10-15 分钟 |
| **总计** | | **20-35 分钟** |

> **注意**：如果任务超时，可以分阶段执行。先完成探索阶段，再执行分析和修复。

---

**Claude Code 特性使用建议**：

- **Step 1-3 探索阶段**：建议使用 **Explore Mode**，让 Claude 自由探索代码库
- **Step 6 分析阶段**：使用 **think** 关键词深度分析问题优先级
- **Step 8 执行前**：可使用 **Plan Mode** 确认修复方案
...
| 优先级 | 标准 | 示例 |
|--------|------|------|
| P0 | 阻塞问题 | 文档引用的文件不存在、命令格式错误 |
| P1 | 功能问题 | 文档与代码不同步、依赖安全漏洞 |
| P2 | 优化建议 | 冗余内容、格式不统一 |
</task>
